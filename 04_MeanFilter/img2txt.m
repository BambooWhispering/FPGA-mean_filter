...
% ---读取源图像文件
% 读取图像文件到图像句柄中
img_src = imread('./img_src/img.jpeg');

% 加高斯噪声
img = imnoise(img_src, 'gaussian', 0, 0.01);
...
fid_rgb = fopen('./img_src/img_rgb.txt', 'wt'); % 以写的方式打开文件，文件不存在则创建
...
imwrite(img_src, './img_src/img.jpeg');
imwrite(img, './img_src/img_guassian.jpeg');
